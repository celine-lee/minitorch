...
# Device helper functions for index calculations.
@cuda.jit(device=True, inline=True)
def _unravel_index_device(cur_ord, shape, out_index, n):
...
@cuda.jit(device=True, inline=True)
def _ravel_index_device(index, strides, n):
    pos = 0
    for i in range(n):
...
def tensor_map(
    ...
    def _map(
        ...
        if i < out_size:
            _unravel_index_device(i, out_shape, out_index, len(out_shape))
            for s in range(len(in_shape)):
                if in_shape[s] > 1:
                    in_index[s] = out_index[s + (len(out_shape) - len(in_shape))]
                else:
                    in_index[s] = 0
            o = _ravel_index_device(out_index, out_strides, len(out_shape))
            j = _ravel_index_device(in_index, in_strides, len(in_shape))
...
def tensor_zip(
    ...
    """
    CUDA higher-order tensor zipWith function.
    """
    def _zip(
        out: Storage,
        out_shape: Shape,
        ...
        if i < out_size:
            ...
            for s in range(len(a_shape)):
                if a_shape[s] > 1:
                    ...
                    a_index[s] = 0
            j = _ravel_index_device(a_index, a_strides, len(a_shape))
            for s in range(len(b_shape)):
                if b_shape[s] > 1:
                    b_index[s] = out_index[s + (len(out_shape) - len(b_shape))]
                else:
                    b_index[s] = 0
            k = _ravel_index_device(b_index, b_strides, len(b_shape))
...
def tensor_reduce(
    ...
    def _reduce(
        ...
        if out_pos < out_size:
            _unravel_index_device(out_pos, out_shape, out_index, len(out_shape))
            o = _ravel_index_device(out_index, out_strides, len(out_shape))
            out_index[reduce_dim] = out_index[reduce_dim] * BLOCK_DIM + pos
            if out_index[reduce_dim] < a_shape[reduce_dim]:
                in_a = _ravel_index_device(out_index, a_strides, len(a_shape))
