...
######################################
# Helper Functions (used in CPU kernels)
######################################
def _compute_multi_index(i: int, shape: Shape, index_out: Index) -> None:
    cur = i
    for idx in range(int(len(shape)) - 1, -1, -1):
        index_out[idx] = int(cur % shape[idx])
        cur //= shape[idx]


def _compute_offset(index: Index, strides: Strides) -> int:
    off = 0
...
######################################
# Protocol and TensorOps base class
######################################
...
######################################
# Low-level implementations (simple versions)
######################################
def tensor_map(fn: Callable[[float], float]) -> Callable[
    [Storage, Shape, Strides, Storage, Shape, Strides], None
]:
    def _map(
        out: Storage,
        out_shape: Shape,
        ...
        for i in range(len(out)):
            cur = i
            _compute_multi_index(cur, out_shape, out_index)
            for s_i, s in enumerate(in_shape):
                if s > 1:
                    in_index[s_i] = out_index[s_i + (len(out_shape) - len(in_shape))]
                else:
                    in_index[s_i] = 0
            o = _compute_offset(out_index, out_strides)
            j = _compute_offset(in_index, in_strides)
    ...
    def _reduce(
        ...
        for i in range(len(out)):
            ...
            for s in range(a_shape[reduce_dim]):
                ...
                j = _compute_offset(out_index, a_strides)
