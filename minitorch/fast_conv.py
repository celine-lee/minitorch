...
# Reusable helper for index conversion for conv kernels.
@njit
def fill_index(flat: int, shape: Tuple[int, ...], out_index: np.ndarray) -> None:
    cur = flat
...
@njit
def compute_position(index: np.ndarray, strides: Tuple[int, ...]) -> int:
    pos = 0
